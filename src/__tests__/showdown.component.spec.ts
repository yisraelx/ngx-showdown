...
describe('ShowdownComponent', () => {
  ...
  it('should be set options (with casting)', () => {
    let fixture = createComponentFixture(showdownComponentModuleMetadata, {
      metadata: {template: '<showdown noHeaderId emoji="false" underline="true" [smartIndentationFix]="true" [headerLevelStart]="2" prefixHeaderId="app-" [options]="options"></showdown>'},
    ...
    expect(showdownComponent.getOption('headerLevelStart')).toBe(2);
    expect(showdownComponent.getOption('prefixHeaderId')).toBe('app-');
  });
